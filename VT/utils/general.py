...
def letterbox(im, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleup=True, stride=32):
    ...
    if auto:  # 最小矩形区域
        dw, dh = np.mod(dw, stride), np.mod(dh, stride)  # 自动补全到stride倍数
...
def save_one_box(xyxy, im, file, gain=1.02, pad=10, square=False):
    """裁剪并保存检测框"""
